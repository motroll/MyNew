...
#define SUB_GR_SELECT_CHAIR				410									//选择座位
#define SUB_GR_JOIN_PRIVATE_SUCESS		411									//加入房间返回
#define SUB_GR_GET_TABLE_INFO			412									//请求坐下人员信息
#define SUB_GR_NOTIFY_GET_PRIVATE		413									//通知前端现在处于选座状态
#define SUB_GR_GET_SELECT_INFO			414									//前端去请求选座信息
#define SUB_GR_GET_MY_ROOM_INFO			415									//前端去请求房间列表信息
#define SUB_GR_ACK_MY_ROOM_INFO			416									//房间列表信息
...
{	
	...
	BYTE							bQiangGangHu;							//抢杠胡，0没有抢杠胡，1有抢杠胡(黄州/黄梅)
	BYTE							bGameCardType;							//棋牌类型，有无风、红中
	BYTE                            bAllOther;								//全求人 0 无全求人 1 有全求人
	BYTE							cbPantherType;							//风豹，将豹，单豹, 色豹
	BYTE                            cbPlayerNum;                            //游戏人数

	//黄石
	BYTE                            bGameTypeIndex;							//2罗田，3黄州黄黄，4溪水，6黄州红中杠,7黄梅，8英山
...
//加入房间结果
struct CMD_GF_Join_Private_Sucess
{
	DWORD							dwRoomNum;								//房间ID
	BYTE							bGameBaseScore;							//房间底分
};

//选择座位
struct CMD_GR_Select_Chair_Private
{
	DWORD							dwRoomNum;								//房间ID
	BYTE							bChairIdex;								//座位号：0东。。。
};

//请求坐下玩家的信息
struct CMD_GR_Get_Room_Chair_Info
{
	DWORD							dwRoomNum;
};

//通知玩家正是选座状态
struct CMD_GR_Notify_Room_Select_States
{
	DWORD							dwTableID;
};

// 玩家请求房间信息
struct CMD_GR_Req_Table_Info
{
	DWORD							dwTableID;
};

// 玩家请求房间信息
struct CMD_GR_Req_My_Room_Info
{
	DWORD							dwUserID;
};

//俱乐部楼层信息
struct  CMD_CF_Ack_Floor_Room
{
	...
	BYTE                            bPatherType;                            //豹子
	BYTE                            bRoomNum;                               //该楼层的房间数
	BYTE                            bSendRoomNum;                           //发送的房间数
};
//每个房间的玩家信息
struct  CMD_CF_Ack_Room_Player
{
	BYTE							cbCount;							    //当前人数
	BYTE							cbState;							    //开始状态
	BYTE                            cbPlayCount;                            //打了多少句
	BYTE                            cbStatus0;                              //1玩家状态
	BYTE                            cbStatus1;								//2玩家状态
	BYTE                            cbStatus2;							    //3玩家状态
	BYTE                            cbStatus3;                              //4玩家状态
	char                            cbUserName0[LEN_NICKNAME];              //1玩家昵称
	char                            cbUserName1[LEN_NICKNAME];				//2玩家昵称
	char                            cbUserName2[LEN_NICKNAME];				//
	char                            cbUserName3[LEN_NICKNAME];
	char                            szHeadHttp0[255];
	char                            szHeadHttp1[255];
	char                            szHeadHttp2[255];
	char                            szHeadHttp3[255];
	DWORD                           dwUserID0;
	DWORD                           dwUserID1;
	DWORD                           dwUserID2;
	DWORD                           dwUserID3;
	DWORD							dwRoomNum;							    //房间号
	SYSTEMTIME                      dwCreateTime;                           //创建时间
};
//房间列表
struct CMD_GF_Ack_My_Room_Info
{
	BYTE							bIsLast;							 //是否最后
	DWORD							dwRoomNum;							 //房间号
	BYTE							cbCount;							 //当前人数
	...
	BYTE                            cbStatus2;
	BYTE                            cbStatus3;
	char                            cbUserName0[LEN_NICKNAME];
	char                            cbUserName1[LEN_NICKNAME];
	char                            cbUserName2[LEN_NICKNAME];
	char                            cbUserName3[LEN_NICKNAME];
	char                            szHeadHttp0[255];
	char                            szHeadHttp1[255];
...
{	
	...
	BYTE			bGameTypeIdex;		//游戏类型
	DWORD			bGameRuleIdex;		//游戏规则
	BYTE			bGamePayType;		//支付类型
	BYTE			bGameBaseScore;		//底分
	BYTE			bGameGuDingMa;		//固定吗
	BYTE			bQiangGangHu;		//抢杠胡，0没有抢杠胡，1有抢杠胡
	BYTE			bStartGame;
	BYTE			bGameCardType;		//类型
	...
	void StreamValue(datastream& kData,bool bSend)
	{
		...
		Stream_VALUE(bGamePayType);
		Stream_VALUE(bGameBaseScore);
		Stream_VALUE(bGameGuDingMa);
		Stream_VALUE(bQiangGangHu);
		Stream_VALUE(bStartGame);
		Stream_VALUE(bGameCardType);
		...
		Stream_VALUE(cbPlayerNum);

		Stream_VALUE(bGameTypeIndex);		//
		Stream_VALUE(bGameTypeQing);		//
		Stream_VALUE(bGameMaxScore);		//
		Stream_VALUE(bGameMaxScoreGold);		//
		Stream_VALUE(bOpenMouthFan);		//
		Stream_VALUE(bQiDuiMengQing);		//

		Stream_VALUE(bFendDing);		//
		Stream_VALUE(JingDing);		//
		Stream_VALUE(bLimitIP);
	}
};
...
{	
	BYTE            bAgent;             //是否代理 0不是  1是
	BYTE			bDismiss;			//解散
	DWORD           dwRoomID;           //房间号
	DWORD           dwClubID;           //解散俱乐部所有房间
};
...
#define SUB_GF_DICE_OPTION			4									//掷骰子选庄
...
#define SUB_GF_ROLL_DICE			103									//掷骰子
...
//俱乐部命令
/////////////////////////////////////////////////////////////////////////////////
#define  SUB_GR_CLUB_ROM_INFO_REQ            501                         //俱乐部房间信息请求
#define  SUB_GR_CLUB_ROOM_INFO_RES           502                         //俱乐部房间信息回应
#define  SUB_GR_CLUB_JOIN_CLUB_ROM_REQ       503                         //进入俱乐部房间请求
#define  SUB_GR_CLUB_INTO_CLUB_TABLE_RES     504                         //进入俱乐部房间回应 
#define  SUB_GR_CLUB_UPDATE_FLOOR_REQ        505                         //更新俱乐部楼层玩法

//俱乐部房间信息请求
struct  CMD_GR_Join_Club_Rom
{
	DWORD           dwClubId;       //俱乐部ID 
	DWORD           dwUserId;		//用户ID
	DWORD           dwUpdateStatus; //更新俱乐部房间
	BYTE            bCreateStatus; //新建状态(新建楼层或者新建俱乐部为1 否则为0)
};

// 进入俱乐部房间请求
struct  CMD_GR_Into_Club_Table_Req
{
	DWORD							dwRoomNum;						  //房间ID
};
// 进入俱乐部房间回应
struct  CMD_GR_Into_Club_Table_Res
{
	DWORD							dwRoomNum;						  //房间ID
	BYTE							bGameBaseScore;					  //房间底分
};
//更新俱乐部楼层
struct CMD_GF_Update_Floor_Stress
{
	BYTE							bFloorIndex;                            //楼层ID
	BYTE							cbGameType;								//游戏类型
	BYTE							bPlayCoutIdex;							//游戏局数
