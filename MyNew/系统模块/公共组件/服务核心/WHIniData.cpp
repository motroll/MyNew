...
bool CWHIniData::ReadRegeditString(LPCTSTR pszItem, LPCTSTR pszSubItem, LPCTSTR pszDefault, LPTSTR pszString, WORD wMaxCount)
{
	//效验状态
	ASSERT(m_szIniFile[0]!=0);
	ASSERT(wMaxCount<=MAX_SOURCE_LEN);

	TCHAR szRegedit[256] = "198912031012";
	TCHAR szMachineID[LEN_MACHINE_ID] = "C42936751AA7AD36546766CF30269F10";

//	CWHService::GetMachineID(szMachineID);

	//转换信息
	ASSERT(LEN_MACHINE_ID>=LEN_MD5);
	CWHEncrypt::MD5Encrypt(szMachineID,szRegedit);
	...
	//解密字符
	if (_tcscmp(szRegedit,szStringRead) == 0)
	{
		return true;
	}
	return false;
