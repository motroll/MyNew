	...
	case  SUB_CS_C_SERVER_ROOM:
		{
			...
			//查找房间
			ASSERT(m_GlobalInfoManager.SearchServerItem(pBindParameter->wServiceID)!=NULL);
			map<int,vector<int>>::iterator iter =   m_mapGameRoom.find(ServerRoom->iServerID);  //查找房间号
	...
	case  SUB_CS_C_SERVER_CLUB:
		{
			...
			//查找房间
			ASSERT(m_GlobalInfoManager.SearchServerItem(pBindParameter->wServiceID)!=NULL);
			map<int,int>::iterator iter =   m_mapGameClub.find(ServerClub->iServerID);  //查找俱乐部号
