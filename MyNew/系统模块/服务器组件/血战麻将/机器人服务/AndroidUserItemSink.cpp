...
#define IDI_OPERATE_CARD			(IDI_REBOT_MODULE_START+2)			//操作牌时间
...
bool  CAndroidUserItemSink::OnEventGameMessage(WORD wSubCmdID, void * pData, WORD wDataSize)
{
	...
	case SUB_S_HUANPAI:
		{
			return OnSubHuanPai(pData,wDataSize);
		}
	case SUB_S_XUANQUE_NOTICE:
		{
			return OnSubXuanQueNotice(pData,wDataSize);
		}
	case SUB_S_XUANQUE:
		{
			return OnSubXuanQue(pData,wDataSize);
		}
	case SUB_S_HUANPAI_CHENGDU:
		{
			return OnSubHuanPaiChengDu(pData,wDataSize);
		}
	case SUB_S_HUANPAI_ANDROID:
		{
			return OnSubHuanPaiAndriod(pData,wDataSize);
		}
	case SUB_S_HUANPAI_NOTICE:
		{
			return true;
		}
...
bool  CAndroidUserItemSink::OnEventSceneMessage(BYTE cbGameStatus, bool bLookonOther, void * pData, WORD wDataSize)
{
	...
	case GS_MJ_FREE:	//空闲状态
		{
			//效验数据
			ASSERT(wDataSize==sizeof(GDMJ_CMD_S_StatusFree));
			if (wDataSize!=sizeof(GDMJ_CMD_S_StatusFree)) return false;

			//变量定义
			GDMJ_CMD_S_StatusFree * pStatusFree=(GDMJ_CMD_S_StatusFree *)pData;
...
{
	//效验数据
	ASSERT(wDataSize==sizeof(GDMJ_CMD_S_GameStart));
	if (wDataSize!=sizeof(GDMJ_CMD_S_GameStart)) return false;

	//变量定义
	GDMJ_CMD_S_GameStart * pGameStart=(GDMJ_CMD_S_GameStart *)pData;
...
{
	//效验数据
	ASSERT(wDataSize==sizeof(GDMJ_CMD_S_OperateNotify));
	if (wDataSize!=sizeof(GDMJ_CMD_S_OperateNotify)) return false;

	//变量定义
	GDMJ_CMD_S_OperateNotify * pOperateNotify=(GDMJ_CMD_S_OperateNotify *)pBuffer;
...
{
	//效验消息
	ASSERT(wDataSize==sizeof(GDMJ_CMD_S_OperateResult));
	if (wDataSize!=sizeof(GDMJ_CMD_S_OperateResult)) return false;

	//消息处理
	GDMJ_CMD_S_OperateResult * pOperateResult=(GDMJ_CMD_S_OperateResult *)pBuffer;
	...
	if ((pOperateResult->cbOperateCode&WIK_GANG)!=0)
	{
		...
		{
			DWORD cbWeaveKind=m_WeaveItemArray[wOperateUser][i].cbWeaveKind;
	...
	else if (pOperateResult->cbOperateCode!=WIK_NULL&&pOperateResult->cbOperateCode!=WIK_CHI_HU)
	{
		...
		DWORD cbWeaveKind=pOperateResult->cbOperateCode;
...
{
	//效验消息
	ASSERT(wDataSize==sizeof(GDMJ_CMD_S_OutCard));
	if (wDataSize!=sizeof(GDMJ_CMD_S_OutCard)) return false;

	//消息处理
	GDMJ_CMD_S_OutCard * pOutCard=(GDMJ_CMD_S_OutCard *)pBuffer;
...
{
	//效验数据
	ASSERT(wDataSize==sizeof(GDMJ_CMD_S_SendCard));
	if (wDataSize!=sizeof(GDMJ_CMD_S_SendCard)) return false;

	//变量定义
	GDMJ_CMD_S_SendCard * pSendCard=(GDMJ_CMD_S_SendCard *)pBuffer;
...
void CAndroidUserItemSink::OnOperateCard( BYTE cbOperateCode,BYTE cbOperateCard )
{
	...
	//发送命令
	GDMJ_CMD_C_OperateCard OperateCard;
	...
	if( (cbCardCount-2)%3 == 0 )
	{
		...
		for (BYTE i=0;i<MAX_INDEX-ZI_PAI_COUNT;i++)
		{
			...
			for (BYTE j=0;j<MAX_INDEX-ZI_PAI_COUNT;j++)
			{
				...
				DWORD cbHuCardKind=m_GameLogic.AnalyseChiHuCard(cbCardIndexTemp,m_WeaveItemArray[wMeChairId],m_cbWeaveCount[wMeChairId],cbCurrentCard,chr);
...
bool CAndroidUserItemSink::OnSubHuanPai(const void * pBuffer, WORD wDataSize)
{
	//效验消息
	ASSERT(wDataSize==sizeof(GDMJ_CMD_S_HuanPai));
	if (wDataSize!=sizeof(GDMJ_CMD_S_HuanPai))return false;

	return true;
}
bool CAndroidUserItemSink::OnSubXuanQueNotice(const void * pBuffer, WORD wDataSize)
{
	return true;
}
bool CAndroidUserItemSink::OnSubXuanQue(const void * pBuffer, WORD wDataSize)
{
	return true;
}
bool CAndroidUserItemSink::OnSubHuanPaiChengDu(const void * pBuffer, WORD wDataSize)
{
	return true;
}

bool CAndroidUserItemSink::OnSubHuanPaiAndriod(const void* pBuffer,WORD wDataSize)
{
	//效验消息
	ASSERT(wDataSize==sizeof(GDMJ_CMD_AndroidCardIndex));
	if (wDataSize!=sizeof(GDMJ_CMD_AndroidCardIndex))return false;

	GDMJ_CMD_AndroidCardIndex* pHuanPai = (GDMJ_CMD_AndroidCardIndex*)pBuffer;
	memcpy(m_cbCardIndex,pHuanPai->cbCardIndex,sizeof(m_cbCardIndex));
	return true;
}
