...
public:
	...
	//设置座位信息
	virtual bool SetUserTableChair(BYTE cbUserStatus, WORD wTableID, WORD wChairID)=NULL;
	//冻结接口
public:
	//冻结积分
...
public:
	...
	//发送数据
	virtual bool SendTableData(WORD wChairID, WORD wSubCmdID, VOID * pData, WORD wDataSize,WORD wMainCmdID=MDM_GF_GAME,bool needReady = true)=NULL;
	//发送数据
...
public:
	...
	//坐下动作
	virtual int PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword=NULL)=NULL;
	//进入选座
	virtual bool PerformSelectChairAction(IServerUserItem * pIServerUserItem)=NULL;
	//未选座状态下切换位置
	virtual bool performExchangeChairAction(IServerUserItem* pIServerUserItem, WORD exchangeChairID)=NULL;

	//功能接口
public:
	//发送场景
	virtual bool SendGameScene(IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize)=NULL;
	virtual void SendGameStatus(VOID* pData, WORD wDataSize)=NULL;

	virtual bool GetTableChairInfo(IServerUserItem * pIServerUserItem) = NULL;
	//私人房
public:
	//设置接口
	...
	//设置私人场信息
	virtual void SetPrivateInfo(BYTE bGameTypeIdex,DWORD	bGameRuleIdex, BYTE bBaseScore, BYTE bGuDingMa, BYTE bQiangGangHu, BYTE bGameType, BYTE bLimitIP)=NULL;
	virtual void SetPrivateFushunInfo(BYTE bGameTypeIndex,BYTE	bGameTypeQing, BYTE bLimitIP,
		BYTE bGameMaxScore,BYTE bGameMaxScoreGold,BYTE bPiaoShu,BYTE bBaoIsOutCard,BYTE bFendDing,BYTE bJingDing,BYTE bAllOther, BYTE	cbPantherType,BYTE	cbPlayerNum, DWORD dwClubID = 0)=NULL;
	virtual void SetCreateUserID(DWORD	dwUserID)=NULL;
	//复位结算信息
	virtual VOID  ResAccountInfo() = NULL ;
	//比赛接口
public:
	//设置接口
...
//static const GUID IID_ITableFrameSink={0x1d79011a,0xbaf9,0x4e52,0xbb,0xc1,0x6e,0xfd,0x12,0xc3,0xa6,0x9b};
...
public:
// 	掷骰子确认庄家
 	virtual bool OnEventDingZhuang(int nChairID)=NULL;
	//掷骰子
	virtual bool OnEventRollSice()=NULL;
	//游戏开始
...
public:
	//设置私人场信息
	virtual void SetPrivateInfo(BYTE bGameTypeIdex,DWORD	bGameRuleIdex, BYTE bBaseScore, BYTE bGuDingMa, BYTE bQiangGangHu, BYTE bGameType)=NULL;
	virtual void SetPrivateFushunInfo(BYTE bGameTypeIndex,BYTE	bGameTypeQing,BYTE bLimitIP,
		BYTE bGameMaxScore,BYTE bGameMaxScoreGold,BYTE bPiaoShu,BYTE bBaoIsOutCard,BYTE bFendDing,BYTE bJingDing,BYTE bAllOther, BYTE	cbPantherType, BYTE	cbPlayerNum, DWORD dwClubID = 0)=NULL;
	virtual void SetCreateUserID(DWORD	dwUserID)=NULL;
	//复位结算信息
