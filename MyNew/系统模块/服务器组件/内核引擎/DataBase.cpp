	...
	try
	{
		...
		switch (vtFld.vt)
		{
			...
			case VT_DECIMAL:
			{
				...
				dbValue /= pow((float)10, (int)vtFld.decVal.scale);
				break;
			}
			case VT_UI1:
			{
...
bool  CDataBase::GetFieldValue(LPCTSTR lpFieldName, DWORD & ulValue)
{
	try
	{
		ulValue = 0L;
		_variant_t vtFld = m_DBRecordset->Fields->GetItem(lpFieldName)->Value;
		if ((vtFld.vt != VT_NULL) && (vtFld.vt != VT_EMPTY)) ulValue = (DWORD)vtFld.ulVal;
		return true;
	}
	catch (CComError & ComError)
	{
...
bool  CDataBase::GetFieldValue(LPCTSTR lpFieldName, __int64 & llValue)
{
	...
	{
		llValue = 0L;
		CString strdata = (LPCTSTR)(_bstr_t)m_DBRecordset->GetCollect(lpFieldName);
		llValue = _atoi64(strdata.GetBuffer());
		return true;
	}
	catch (CComError & ComError)
	{
...
{
	for (LONG i = 0;i<wSinkCount;i++)
	{
		//查询接口
		m_pIDataBaseEngineSink = QUERY_OBJECT_PTR_INTERFACE(pIUnknownEx[i], IDataBaseEngineSink);
