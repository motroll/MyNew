...
#define IDI_CHECK_PAY_INFO		    (IDI_MAIN_MODULE_START+11)			//每分钟检查支付
...
#define TIME_CHECK_PAY_INFO			30L									//每分钟检查支付
...
bool CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam)
{
	...
	if ((dwTimerID>=IDI_MAIN_MODULE_START)&&(dwTimerID<=IDI_MAIN_MODULE_FINISH))
	{
		...
		case IDI_CHECK_PAY_INFO://支付
			{
				//投递请求
				m_pIRecordDataBaseEngine->PostDataBaseRequest(DBR_GR_CHECK_PAY_INFO,0,NULL,0);				
				return true;
			}
			break;
...
bool CAttemperEngineSink::OnEventTCPNetworkShut(DWORD dwClientAddr, DWORD dwActiveTime, DWORD dwSocketID)
{
	...
	if (pIServerUserItem!=NULL)
	{
		...
		if (wTableID!=INVALID_TABLE)
		{
			...
			if(pIServerUserItem->GetUserStatus() > US_SELECT)
...
bool CAttemperEngineSink::OnEventUserItemStatus(IServerUserItem * pIServerUserItem, WORD wOldTableID, WORD wOldChairID)
{
		...
		((m_pGameServiceOption->wServerType&GAME_GENRE_MATCH)!=0 || (m_pGameServiceOption->wServerType&GAME_GENRE_EDUCATE)!=0))
	{
		...
		{
			pTableFrame->StartGame(pIServerUserItem->GetChairID()); 
		}
	}
	return true;
}
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	lstrcpyn(UserInfo.szHeadHttp,pDBOLogonSuccess->szHeadHttp,CountArray(UserInfo.szHeadHttp));
...
bool CAttemperEngineSink::OnDBUserPaySuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	dwContextID = pIServerUserItem->GetBindIndex();
	...
	CMD_GP_UserInsureSuccess UserInsureSuccess_2;
	ZeroMemory(&UserInsureSuccess_2,sizeof(UserInsureSuccess_2));

	//设置变量
	UserInsureSuccess_2.dwUserID=pUserInsureSuccess->dwUserID;
	UserInsureSuccess_2.lUserScore=pIServerUserItem->GetUserScore()+pIServerUserItem->GetTrusteeScore();
	UserInsureSuccess_2.lUserInsure=pUserInsureSuccess->lSourceInsure+pUserInsureSuccess->lVariationInsure;
	lstrcpyn(UserInsureSuccess_2.szDescribeString,pUserInsureSuccess->szDescribeString,CountArray(UserInsureSuccess_2.szDescribeString));

	//发送消息
	WORD wDescribe=CountStringBuffer(UserInsureSuccess_2.szDescribeString);
	WORD wHeadSize=sizeof(UserInsureSuccess_2)-sizeof(UserInsureSuccess_2.szDescribeString);
	m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_USER_SERVICE,SUB_GP_USER_INSURE_SUCCESS,&UserInsureSuccess_2,wHeadSize+wDescribe);
...
bool CAttemperEngineSink::OnDBPropertySuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if(pISourceUserItem == NULL || pITargetUserItem == NULL)
		return false;
...
// 		return true;
	...
	if(pISourceUserItem)
	{
		...
		if(pTableFrame)
		{
			...
			{
				IServerUserItem* pUserItem = pTableFrame->GetTableUserItem(iUser);
				//if (pUserItem)
				{
					SendData(pUserItem,MDM_GR_USER,SUB_GR_PROPERTY_SUCCESS,&PropertySuccess,sizeof(PropertySuccess));
				}
			}
		}
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubUserStandUp(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if(pUserStandUp->bStatuse > US_SIT)
	{
...
int CAttemperEngineSink::OnPropertyBuy(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//练习房间
	if (false && m_pGameServiceOption->wServerType==GAME_GENRE_EDUCATE)
	{
	...
	if (pPropertyBuy->cbConsumeScore==TRUE)
	{
		...
		if (false && (m_pGameServiceOption->wServerType&GAME_GENRE_GOLD)==0) return RESULT_FAIL;		
...
bool CAttemperEngineSink::SwitchUserItemConnect(IServerUserItem * pIServerUserItem, TCHAR szMachineID[LEN_MACHINE_ID], WORD wTargetIndex,BYTE cbDeviceType,WORD wBehaviorFlags,WORD wPageTableCount)
{
	...
	else if(pIServerUserItem->GetUserStatus()==US_SELECT)
	{
...
//创建私人场通知
void  CAttemperEngineSink::SendPrivateRoom(int  iRoomId, bool bAddFlag)
{
	CMD_CS_S_ServerRoom  ServerRoom;
	ZeroMemory(&ServerRoom,sizeof(ServerRoom));
	ServerRoom.dwRoomID  = iRoomId;   //房间号
	ServerRoom.bAddFlag  =  bAddFlag;
	ServerRoom.iServerID = m_pGameServiceOption->wServerID;
	m_pITCPSocketService->SendData(MDM_CS_SERVICE_INFO,SUB_CS_C_SERVER_ONLINE,&ServerRoom,sizeof(ServerRoom));
}
//创建俱乐部通知
void  CAttemperEngineSink::SendPrivateClub(std::vector<CMD_CS_S_ServerRoom> vecServerRoom, int  iClubId, bool bAddFlag)
{
	CMD_CS_S_ServerClub  ServerClub;
	ZeroMemory(&ServerClub,sizeof(ServerClub));
	ServerClub.dwClubID  = iClubId;   //房间号
	ServerClub.bAddFlag  =  bAddFlag;
	ServerClub.bRoomNum = vecServerRoom.size();
	ServerClub.iServerID = m_pGameServiceOption->wServerID;
	m_pITCPSocketService->SendData(MDM_CS_SERVICE_INFO,SUB_CS_C_SERVER_ONLINE,&ServerClub,sizeof(ServerClub));
}
