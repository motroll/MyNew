...
CTableFrame::CTableFrame()
{
	...
	ZeroMemory(m_nSitDownSt, sizeof(m_nSitDownSt));
	ZeroMemory(m_nZhuang, sizeof(m_nZhuang));
	ZeroMemory(m_nDice, sizeof(m_nDice));
...
//开始游戏
bool CTableFrame::StartGame(WORD chairID)
{
	//游戏状态
	...
	if(m_pITableFrameSink && !m_bDingZhuang && !m_pITableFrameSink->OnEventDingZhuang(chairID))
	{
		m_bDingZhuang = true;
		m_DiceTimes = 0;
		m_iRollCount = 0;
	...
	// 后续就是掷骰子
	if(m_pITableFrameSink && !m_bRollDice && !m_pITableFrameSink->OnEventRollSice())
	{
		m_bRollDice = true;
		m_iRollCount=0;
		ZeroMemory(m_nDice, sizeof(m_nDice));
	...
	//old
// 	if(m_pITableFrameSink && !m_bDingZhuang && !m_pITableFrameSink->OnEventDingZhuang())
// 	{
// 		m_bDingZhuang = true;
// 		m_iRollCount = 0;
// 		return false;
...
//结束整个房间
bool CTableFrame::TableFrameEnd()
{
	m_bDingZhuang = false;
	m_DiceTimes = 0;
	m_bRollDice = false;
...
//发送数据
bool CTableFrame::SendTableData(WORD wChairID, WORD wSubCmdID, VOID * pData, WORD wDataSize,WORD wMainCmdID, bool needReady)
{
	//用户群发
	if(needReady)
	{
...
//设置私人场信息
void CTableFrame::SetPrivateFushunInfo(BYTE bGameTypeIndex,BYTE bGameTypeQing,BYTE bLimitIP,BYTE bGameMaxScore,BYTE bGameMaxScoreGold,BYTE bPiaoShu,BYTE bBaoIsOutCard,BYTE bFendDing,BYTE bJingDing,BYTE bAllOther, BYTE	cbPantherType, BYTE cbPlayerNum,DWORD dwClubID)
{
	m_bLimitIP = bLimitIP;
	m_wChairCount = cbPlayerNum; 
	if (m_pITableFrameSink)
	{
		m_pITableFrameSink->SetPrivateFushunInfo(bGameTypeIndex,bGameTypeQing,bLimitIP, bGameMaxScore, bGameMaxScoreGold, bPiaoShu, bBaoIsOutCard, bFendDing, bJingDing,bAllOther,cbPantherType,cbPlayerNum,dwClubID);
	}
}

//设置私人场信息
void CTableFrame::SetPrivateInfo(BYTE bGameTypeIdex,DWORD bGameRuleIdex, BYTE bBaseScore, BYTE bGuDingMa, BYTE bQiangGangHu, BYTE bGameType, BYTE bLimitIP)
{
	if (m_pITableFrameSink)
	{
		m_pITableFrameSink->SetPrivateInfo(bGameTypeIdex,bGameRuleIdex,bBaseScore, bGuDingMa, bQiangGangHu, bGameType);
	}
}
//结算信息  
void   CTableFrame::ResAccountInfo()
{
	if (m_pITableFrameSink)
	{
		m_pITableFrameSink->ResAccountInfo();
	}
}	
...
bool CTableFrame::GetTableChairInfo(IServerUserItem * pIServerUserItem)
{
	for (WORD i=0;i<m_wChairCount;i++)
	{
		//获取用户
...
void CTableFrame::SendGameStatus(VOID* pData, WORD wDataSize)
{
	//发送状态
	for (WORD i=0;i<m_wChairCount;i++)
	{
		...
		GameStatus.cbAllowLookon=m_bAllowLookon[wChairID]?TRUE:FALSE;
		m_pIMainServiceFrame->SendData(pIServerUserItem,MDM_GF_FRAME,SUB_GF_GAME_STATUS,&GameStatus,sizeof(GameStatus));
		SendGameScene(pIServerUserItem,pData,wDataSize);
	}

}

bool CTableFrame::updatePlayerStatus()
{
...
bool CTableFrame::PerformStandUpActionEx(IServerUserItem * pIServerUserItem)
{
	...
	if (pIServerUserItem==pITableUserItem)
	{
		...
		{
			StartGame(pIServerUserItem->GetChairID());
		}
...
bool CTableFrame::PerformSelectChairAction(IServerUserItem * pIServerUserItem)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
	ASSERT(pIServerUserItem->GetTableID()==m_wTableID);
	...
	if(wChairID == INVALID_CHAIR)
	{
		...
		{
			bg = offset + i;
			if(GetTableUserItem(bg) == NULL)
				break;
		}
		m_TableUserItemArray[bg]=pIServerUserItem;
		wChairID = bg;
	}
	...
	if ((IsGameStarted()==false)||(m_cbStartMode!=START_MODE_TIME_CONTROL))
	{
		...
		{
			pIServerUserItem->SetClientReady(false);
//			pIServerUserItem->SetUserStatus(US_FREE,m_wTableID,wChairID);
			pIServerUserItem->SetUserTableChair(US_SELECT,m_wTableID,wChairID);
		}
		else
		{
			pIServerUserItem->SetClientReady(false);
...
//未选座状态下切换位置
bool CTableFrame::performExchangeChairAction(IServerUserItem* pIServerUserItem, WORD exchangeChairID)
{
	ASSERT(pIServerUserItem!=NULL);
	ASSERT(pIServerUserItem->GetTableID()==m_wTableID);
	ASSERT(pIServerUserItem->GetChairID()<=m_wChairCount);
	...
	if(exchangeChairID == INVALID_CHAIR)
		return false;
	...
	//设置变量
		...
		m_TableUserItemArray[exchangeChairID]=pIServerUserItem;
		wChairID = exchangeChairID;
...
//坐下动作
int CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword)
{
	//效验参数
	ASSERT((pIServerUserItem!=NULL)&&(wChairID<m_wChairCount));
	ASSERT((pIServerUserItem->GetTableID()==INVALID_TABLE)&&(pIServerUserItem->GetChairID()==INVALID_CHAIR));
	...
	tagUserInfo * pUserInfo=pIServerUserItem->GetUserInfo();
	tagUserRule * pUserRule=pIServerUserItem->GetUserRule();
	int oldChairID = pIServerUserItem->GetChairID();
	IServerUserItem * pITableUserItem=GetTableUserItem(wChairID);
...
// 	}
	...
	//规则效验
	if (EfficacyIPAddress(pIServerUserItem)==false) return 2;
	if (EfficacyScoreRule(pIServerUserItem)==false) return false;
	...
	//设置变量
	if(oldChairID != INVALID_CHAIR)
	{
		m_TableUserItemArray[oldChairID] = NULL;
		m_nSitDownSt[oldChairID] = 0;
	}
	m_TableUserItemArray[wChairID]=pIServerUserItem;
	m_nSitDownSt[wChairID] = 1;
...
bool CTableFrame::EfficacyStartGame(WORD wReadyChairID)
{
	...
	if(wReadyChairID != INVALID_CHAIR)
	{
		...
		if(m_bRollDice)
		{
			if(m_nDice[wReadyChairID] != 1)
			{
				m_nDice[wReadyChairID] = 1;
			}
		}
		else if(m_bDingZhuang)//定庄
		{
			if(m_nZhuang[wReadyChairID] != 1)
			{
				m_nZhuang[wReadyChairID] = 1;
			}
		}
	}
...
//			if(m_DiceTimes > 0)
			...
			if(pITableUserItem->IsClientReady())
				m_readyNum++;
		}
	}
	...
	if (bCheckSameIP==true)
	{
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if ((pITableUserItem!=NULL)&&(pITableUserItem != pIServerUserItem)&&(!pITableUserItem->IsAndroidUser())&&(pITableUserItem->GetMasterOrder()==0)&&(pITableUserItem->GetClientAddr()==dwUserIP))
			{
				...
				{
					//发送信息
					LPCTSTR pszDescribe=TEXT("同ip用户不能同时进房");
